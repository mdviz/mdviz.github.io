...
function createNodes(nodes){
    ...
    jsonNodes = [];
    for (obj in nodes){
        jsonNodes.push({id:obj, flowsO: nnodes[obj].flowsO, flowsD : nnodes[obj].flowsD, classId:nnodes[obj].classId})
    }

    top10destination = jsonNodes.sort(function(a,b){return a.flowsD < b.flowsD ? 1 : -1; }).slice(0,20);
    ...
    sGlobals.svg.selectAll("circle")
        ...
        .on('mouseover', function(){
            ...
            d3.selectAll(".bz:not(." + nodeDID +")").classed('myhidden', true);
            d3.selectAll("."+ nodeDID).classed('highlight', true);
        ...
        .on("mouseout", function(){
            console.log(this)
            var nodeDID = "O_"+this.classList[0];
            d3.select(".N_" + nodeDID).attr("r", 2)
            d3.selectAll(".bz").classed('myhidden', false);
    ...
    textLabels
        ...
        .text(function(d) {
            return d3.format("0,000")(d.flowsD)
        })
